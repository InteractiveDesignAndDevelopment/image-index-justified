<cfcomponent accessors="true" output="true">
  ...
  <cfscript>
    ...
    public function booleanClassOf(required string klass, trueClass, falseClass) {
      if (booleanOf(klass)) {
        if (isDefined('trueClass')) {
          return trueClass;
        } else {
          return klass;
        }
      } else {
        if (isDefined('falseClass')) {
          return falseClass;
        } else {
          return '';
        }
    ...
    public function booleanOf(required string klass) {
      return 0 lt ArrayFind(classes, klass);
    }
